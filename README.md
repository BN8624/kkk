...
<p align="center"><img src="docs/hero.gif" alt="세 왕관의 섬 2.1 플레이 흐름" width="360"></p>
...
**호환 정책:** 1.5.x·2.0.x 비-crown 리플레이는 exact 재생. 2.0.x crown-heart 는 왕관 규칙 변경으로 재생 전용(playable-unverified). 2.1.x 는 exact.
...
npm run release:assets # dist 기반 2.1 업로드 묶음과 SHA-256 체크섬 생성
```
...
- 게임 설명 초안: *하나의 섬, 세 왕국. 빠른 전투·9개 미션 캠페인·모바일 전장 제작실·결정론적 리플레이·플레이 분석·일일 도전을 브라우저에서 바로 플레이. 왕관의 심장은 3라운드 활성화 뒤 4턴 연속 확보로 승리하며 경합 시 정지가 적용된다. 한국어·영어, 오프라인 PWA.*
...
Three Crowns Island is a mobile-first, deterministic hex strategy game and scenario workshop. Version 2.1 includes quick battles, nine campaign missions, a daily challenge, six official custom battlefields, a mobile editor, replay playback and local playtest analysis, Korean/English UI, offline PWA play, full/selective data backup, and a fairness fix for Heart of the Crown (activation on round 3, contested-hold pause, arrival-fair maps).
...
- Existing 1.5 saves remain supported. Replay compatibility: 1.5.x and 2.0.x non-crown are exact; 2.0.x crown-heart is playable-unverified (rules changed); 2.1.x is exact.
- For contributors: run `npm ci`, `npm run lint`, `npm run typecheck`, `npm test`, and `npm run test:e2e:dist` before submitting changes.

See [the 2.1 release notes](docs/RELEASE_NOTES_2.1.0.md) and [the physical iPhone checklist](docs/IPHONE_CHECKLIST.md) for release details.
