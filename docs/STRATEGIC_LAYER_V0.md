# Strategic Layer V0 (Phase 8-0 · 8-1 · 8-2 · 8-3)
...
| **Phase 8-3** | 예시 화면 기준 전략 지도 시각 전면 재구축(자연 섬·거점·깃발 군단·compact HUD) |
...
## Phase 8-2 / 8-3 섬 영토 지도 렌더러

### 모듈

```
src/ui/strategic/
  map-geometry.ts   # 12영토 자연 해안 path·앵커·강/도로 정본 (viewBox 0 0 360 480 세로)
  map-icons.ts      # 소유 soft tint·거점 랜드마크·지형 decor·군단 깃발
  map-view.ts       # 섬 SVG 조립·전선·바인딩
  map-panel.ts      # 하단 선택 시트
  map-animation.ts  # 이동 경로·애니메이션·플래시
  styles.ts         # compact HUD/지도/패널 레이아웃
  index.ts / screens.ts
```
...
- 각 지역은 고정 `path` + `labelAnchor` + `structureAnchor` + `armyAnchors` + `edgeMidpoints` + `coastal`.
- `edgeMidpoints` 키 집합은 정본 이웃과 1:1 일치해야 한다(단위 테스트).
- 시각은 퍼즐 격자·동일 크기 polygon이 아닌 유기 해안·내륙 형태.
- 랜덤 지도 생성 없음. 예시 이미지 비트맵을 게임 자산으로 쓰지 않음.

### 렌더 계약 (Phase 8-3)

| 항상 표시 | 선택 시에만 (하단 시트) |
| --- | --- |
| 자연 섬 실루엣·외해 해안·여울 | 지역 이름·수입·방어·주둔 |
| 지형 베이스색 + soft 소유 틴트(강한 사선/격자 패턴 없음) | 병종 HP 목록 |
| 숲/산/평야 decor, 강, 도로 | 대기/보충 명령 |
| 수도/도시/요새 랜드마크 | |
| 군단 깃발 토큰(문양·병력·HP바·행동완료 — A/C/V 문자 마커 아님) | |
| 적대 전선, 수도 위협 링, 선택/이동 강조 | |
...
- compact HUD 1줄(문장·턴·국고·영토·군단·턴 종료·메뉴), 높이 ≤15%
- 지도 폭 ≥92%, 힌트 패널 시 지도 높이 ≥70%
- 기본 하단은 짧은 힌트(사이안 안내 마키 없음)
...
- `mobile-first.png`, `army-selected.png`, `move-highlight.png`, `move-done.png`, `pc-map.png`, `battle-return-or-tactical.png` / `after-turn.png`
