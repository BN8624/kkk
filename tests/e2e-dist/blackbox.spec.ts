...
test('데이터 백업 다운로드와 복구 미리보기·병합', async ({ page }) => {
  await page.goto('/');
  await page.getByRole('button', { name: '데이터 백업·복구' }).click();
  await expect(page.getByLabel('설정·언어·즐겨찾기')).toBeChecked();
  await expect(page.getByLabel('리플레이·플레이 분석 메타데이터')).toBeChecked();

  const downloadPromise = page.waitForEvent('download');
  await page.getByRole('button', { name: '선택 데이터 백업' }).click();
  const download = await downloadPromise;
  expect(download.suggestedFilename()).toMatch(/^three-crowns-backup-\d{4}-\d{2}-\d{2}\.json$/);

  const backup = {
    schemaVersion: 1,
    product: 'three-crowns-island',
    createdAt: '2026-07-19T00:00:00.000Z',
    categories: ['preferences'],
    localStorage: { 'three-crowns-locale': 'en' },
    documents: {},
  };
  ...
  await expect(page.getByRole('heading', { name: '복구 미리보기' })).toBeVisible();
  await expect(page.getByText('로컬 항목 1개 · 문서 0개')).toBeVisible();
