...
import { encodeShareCode } from '../../src/editor/share';
...
test('에디터: 공유 코드 붙여넣기 가져오기', async ({ page }) => {
  ...
  await page.locator('#ed-import-text').fill(code);
  await page.locator('#btn-import-go').click();
  await expect(page.locator('.ed-palette')).toBeVisible();
  const opened = (await page.evaluate(() => window.__tc!.editorDoc!()))!;
  expect(opened.title).toBe('공유 E2E');
  // 가져온 문서는 새 ID를 받아 원본을 덮어쓰지 않는다
  expect(opened.id).not.toBe('custom-e2e');
});

test('에디터: 공유 URL(#s=코드)로 진입하면 제작실이 열린다', async ({ page }) => {
  const doc = cloneBuiltinDocument('broken-strait', 'custom-url', 12, '공유 URL E2E');
  const code = await encodeShareCode(doc);
  page.on('dialog', (d) => void d.accept());
  await page.goto(`/#s=${code}`);
  await expect(page.locator('.ed-palette')).toBeVisible();
  const opened = (await page.evaluate(() => window.__tc!.editorDoc!()))!;
  expect(opened.title).toBe('공유 URL E2E');
  // 해시는 소비 후 제거되어 새로 고침 시 재가져오기가 없다
