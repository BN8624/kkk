...
test('유닛 패널: 수호대 선택 시 수호 태세 문구와 활성 상태가 보인다', async ({ page }) => {
  await startNewGame(page, 13);
  // 수호대 비용 48. 왕관 과우위 완화로 azure 시작 금이 40→32로 낮아져(capital 수입 15/턴)
  // 1라운드(32→47)로는 1골드 부족하고 2라운드(→62) 수입 후 생산 가능하다. azure 턴을 2번 넘긴다.
  for (const targetTurn of [2, 3]) {
    await page.getByRole('button', { name: '턴 종료' }).click();
    await page.waitForFunction(
      (t) => {
        const s = window.__tc?.state();
        return s !== null && s !== undefined && s.current === 'azure' && s.turn >= t;
      },
      targetTurn,
      { timeout: 120_000 },
    );
